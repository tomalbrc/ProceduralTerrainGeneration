		...
		if (!CreationParams.Fullscreen)
		{
			...
			if (Window != NULL || CreationParams.WindowId)
			{
				if (CreationParams.DriverType == video::EDT_OPENGL)
				{
					...
					if (format != NULL)
					{
                        NSOpenGLView *glView = [[NSOpenGLView alloc] initWithFrame:NSMakeRect(0, 0, 400, 400) pixelFormat:format];
                        
                        Window.contentView = glView;
                        
                        OGLContext = glView.openGLContext;//[[NSOpenGLContext alloc] initWithFormat:format shareContext:NULL];
						[format release];
					}
				}
