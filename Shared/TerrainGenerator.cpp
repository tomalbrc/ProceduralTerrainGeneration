...
//	_image = image;
    ...
    tom::threading::addMainCallback([m_quadScale = m_quadScale, key = std::move(chunkLocation), offset = std::move(offset), m_device = m_device, imageHeightmap = imageHeightmap, m_terrainHeight = m_terrainHeight, m_chunkSize = m_chunkSize, completion = std::move(completion)]() mutable {
        ...
        msn->setMaterialTexture(0, m_device->getVideoDriver()->getTexture(ResourcePath("models/sand.png")));
        msn->setMaterialTexture(1, m_device->getVideoDriver()->getTexture(ResourcePath("models/grass.png")));
        msn->setMaterialTexture(2, m_device->getVideoDriver()->getTexture(ResourcePath("models/rock.png")));
        msn->setMaterialTexture(3, m_device->getVideoDriver()->getTexture(ResourcePath("models/snow.png")));
